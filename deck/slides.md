...
| **가이드가 실측으로** | `확인 필요(따라가기)` 표시 **92곳 → 60곳** · 막힌 곳과 우회를 각 단계 장에 그대로 적었습니다 |
...
| 남은 일 | 그러면 무엇이 바뀌나 |
|---|---|
| ✅ 새 설치본으로 **S0~S8 을 따라가 봄** — 1차 완료(2026-09-13~16) | 연결 27개에 `검증됨`과 확인일이 붙었고, `확인 필요(따라가기)` 표시가 92 → 60곳으로 줄었습니다 |
