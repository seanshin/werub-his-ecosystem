...
| 언제 기준 | 2026-09-11 계측 · 판정 · **2026-09-13~16 새 설치본 따라가기**(연결 `검증됨` 27) |
...
| 표시 | 뜻 |
|---|---|
| `검증됨` | 실제로 호출해 동작을 확인함(확인일 필수) — **이 초안에는 27개** — 목록은 [따라가 본 결과](../build-guide/follow-along-2026-09.md#실제로-호출해-검증됨-을-붙인-연결) 한 곳에 있습니다 |
...
| ③ 진료(AI 보조) | HIS → AI Server — **의료진이 승인해야 기록이 됩니다** | `구현·미검증`(요약 경로 하나만 불러 봄) |
| ④ 검사 | HIS ⇄ LIS (FHIR R4 ServiceRequest · DiagnosticReport) | **`검증됨` 5**(오더 · 환자 조회 · 결과 · 취소 · 검사코드 카탈로그 · 2026-09-14~15) |
| ⑤⑥ 영상 | LIS → PACS(HL7 v2 · 병리) | **`검증됨` 2**(워크리스트 · 뷰어 링크 · 2026-09-15) |
| ⑤⑥ 영상 | HIS → PACS · PACS → 촬영 장비(DICOM) | `구현·미검증` — HIS 화면에서 PACS 를 부르는 경로는 **끝까지 가지 못했습니다** |
| ⑦⑧ 서명 | PACS → sign · HIS ⇄ sign | **`검증됨` 5**(판독 서명 · 조영제 동의서 · 직원 서명 · 완료 통지 · 오더 로그 봉인) |
| ⑨ 수납 · 청구 | HIS ⇄ ERP | **`검증됨` 6**(직원 SSO · 진료비 계산서 · 약품 보험코드 · 청구 라인 · 정산 회신 · 계약 서명 발의) |
| ⑨ 수납 · 청구 | HIS → ERP 운영 이벤트(수납 · 청구 · 재고 · 인사) | `구현·미검증` — **리얼 전환 뒤에** 실제로 나갑니다 |
...
| | |
|---|---|
| **1** | **첫 기동은 외부로 나가는 연결을 막은 상태에서 합니다.** 각 시스템의 코드 · 설정 예시에 특정 설치본의 주소가 기본값으로 든 파일이 **11개 저장소 합계 299개 — [파일 목록](../build-guide/replace-list.md)**(문서 제외 · 2026-09-17 기준 커밋에서 다시 셈) 있습니다. 바꾸지 않고 띄우면 **다른 설치본으로 요청이 갈 수 있습니다.** |
...
**덧붙여** — HIS 코드에 병원명이 고정 문자열로 남은 파일이 **118개** 있습니다(2026-09-17 기준 커밋에서 다시 셈에서 다시 셈). 설정값으로 옮기는 작업이 진행 중이며, 끝나기 전까지 자기 병원명을 넣으려면 코드를 고칩니다(S1).
...
## 따라가 보고 확인된 것 — 설치본 7개를 세워서

**기준 커밋으로 새로 세운 설치본**(HIS · sign · LIS · PACS · ERP · edu · AI Server)을 **외부로 나가지 못하는 네트워크**에 올리고, 새 설치본끼리만 실제로 불렀습니다.

| 무엇 | 확인된 것 |
|---|---|
| **연결 27개가 끝까지** | 검사 오더 → 결과 → 취소, 직원 · 환자 전자서명, 판독 서명, 진료비 계산서, 청구 · 정산, 사내교육 이수까지 **한 줄로 이어졌습니다** |
| **가드가 실제로 막았다** | 27개 중 **23개**에 틀린 키 · 변조 토큰 · 위조 서명 · 재전송 · 다른 대상을 넣어 다시 시험 → 전부 거부(401 · 403 · 409) · 나머지 4개는 거부를 시험할 자리가 없어 정상 동작만 |
| **리허설 모드가 잡아 둔다** | 문자 · 메일 · 연동 발송이 나가지 않고 대기열에 보류됐고, 보류 중이라는 사실을 로그로 계속 알렸습니다 |
| **백업이 복원됐다** | 암호화 백업을 다른 DB 에 복원해 **562개 테이블의 행 수 일치**까지 확인 |
| **원본 그대로 되는 설치** | sign · ERP · edu 는 저장소의 운영용 이미지가 **그대로 빌드되고 떴습니다** |
| **가이드가 실측으로** | `확인 필요(따라가기)` 표시 **92곳 → **22곳**** · 막힌 곳과 우회를 각 단계 장에 그대로 적었습니다 |

> ⚠️ **기준 장비가 아닙니다.** 개발 PC 한 대(arm64 · GPU 없음)였습니다. **처리량 · 응답 시간 · AI 품질은 이 결과로 판단하지 않습니다.**
> 끝까지 가지 못한 연결 · 코드에 고정돼 설정으로 못 바꾸는 경로 · 아직 하지 않은 것(기준 장비 · 리얼 전환 · 미설치 시스템)도 **빠짐없이 적어 두었습니다.**

근거: [따라가 본 결과](../build-guide/follow-along-2026-09.md)
...
| 남은 일 | 그러면 무엇이 바뀌나 |
|---|---|
| ✅ 새 설치본으로 **S0~S8 을 따라가 봄** — 1차 완료(2026-09-13~16) | 연결 27개에 `검증됨`과 확인일이 붙었고, `확인 필요(따라가기)` 표시가 92 → 22곳으로 줄었습니다 |
...
>
> **그리고 지금, 실제로 호출해 확인한 연결은 27개입니다** — 새 설치본끼리, 그중 23개는 **가드가 막는 것까지** 확인했습니다.
> 코드만 맞물린 연결이 61개 남아 있고, 기준 장비 · 리얼 전환 · 아직 세우지 않은 시스템은 다음 차례입니다.
>
