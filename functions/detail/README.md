...
| [검체 — 「이 검체로는 못 낸다」를 말하는 자리](specimen-lifecycle.md) | ① 정직성 · ② 정본 · ⑥ 사람 결정 | 🔴 시스템 안 + 장비 경로는 `미구현` |
| [직원 자격·교육 — 빈 원장을 「이상 없음」으로 말하지 않는다](staff-credentials.md) | ① 정직성 · ② 정본 · ⑦ 수치 | 🟢 [연결 표](../../RELEASES/2026.09/compatibility.md) — 연결 6개 `검증됨` |
| [퇴원 게이트 — 면제도 기록에 남긴다](discharge-gate.md) | ① 정직성 · ② 정본 · ④ 게이트 · ⑥ 사람 결정 | 🔴 시스템 안(연결 표에 없음) |
| [위원회 의결 — 정한 것이 시스템에 반영되는 길](governance-enactment.md) | ② 정본 · ④ 게이트 · ⑤ 권한 · ⑥ 사람 결정 | 🔴 시스템 안(연결 표에 없음) |
| [방사선 치료 — 몸에 들어간 선량은 상태 라벨과 무관하다](radiation-therapy.md) | ① 정직성 · ③ 가드 · ④ 게이트 · ⑤ 권한 | 🔴 시스템 안(연결 표에 없음) |
| [환자에게 닿기 전 마지막 관문 — 재고 · 장비 · 멸균](supply-equipment-sterile.md) | ① 정직성 · ③ 가드 · ④ 게이트 | 🔴 시스템 안(연결 표에 없음) |
| [격리·강박과 비자의입원 — 법이 시간을 정해 둔 자리](seclusion-restraint.md) | ① 정직성 · ④ 게이트 · ⑤ 권한 · ⑥ 사람 결정 | 🔴 시스템 안(연결 표에 없음) |
| [환자 상태 점수 — 못 낸 점수를 0점으로 내지 않는다](clinical-scores.md) | ① 정직성 · ② 정본 · ⑦ 수치 | 🔴 시스템 안(연결 표에 없음) |
| [환자 확인 — 이름만으로 사람을 특정하지 않는다](patient-identification.md) | ① 정직성 · ③ 가드 · ④ 게이트 | 🔴 시스템 안(연결 표에 없음) |
| [항암 치료 — 계산을 보여 주고, 없는 결과를 통과로 접지 않는다](chemotherapy.md) | ① 정직성 · ③ 가드 · ⑤ 권한 · ⑦ 수치 | 🔴 시스템 안(연결 표에 없음) |
| [투석 — 물을 빼는 속도에 상한을 두고, 적절도는 실측만 남긴다](dialysis.md) | ① 정직성 · ③ 가드 · ④ 게이트 · ⑦ 수치 | 🔴 시스템 안(연결 표에 없음) |
| [응급 중증도 분류 — 분류한 사실을 남기고, 0분을 성적으로 세지 않는다](emergency-triage.md) | ① 정직성 · ② 정본 · ④ 게이트 · ⑦ 수치 | 🔴 시스템 안(연결 표에 없음) |
| [외래 예약·대기 — 아직 오지 않은 사람을 지금 기다리는 사람과 섞지 않는다](outpatient-scheduling.md) | ① 정직성 · ④ 게이트 · ⑥ 대외 발신 · ⑦ 수치 | 🔴 시스템 안(환자 셀프 예약만 연결 표에 있음) |
| [원내 처방집 — 켤 수 있다고 해서 막히는 것은 아니다](formulary.md) | ① 정직성 · ② 정본 · ④ 게이트 · ⑥ 사람 결정 | 🔴 시스템 안(연결 표에 없음) |

## 아직 쓰지 않은 것

DUR 차단·경고 · 영상 판독 서명 · 응급 여정 프로토콜 게이트 · 진단서·기록 사본 발급 — 🔴 **빈 틀을 만들지 않고, 쓸 때 만듭니다.**
