...
| ④ | 검사 오더 | HIS → LIS | LIS 가 HIS 의 FHIR R4 검사 오더(ServiceRequest)를 주기적으로 가져감 · 취소도 같은 방식 | `검증됨`(2026-09-14 · 취소 전파 2026-09-15) |
| ④ | 검사 결과 | LIS → HIS | 결과 확정 시 FHIR DiagnosticReport 로 보냄 → HIS 에서 확인 뒤 반영 | `검증됨`(2026-09-15) |
| ⑤ | 병리 영상 | LIS → PACS | 병리 슬라이드 스캔 워크리스트 등록 · 취소(HL7 v2 · MLLP) · 뷰어 링크 | `검증됨`(2026-09-15) |
| ⑥ | 영상 오더 | HIS → PACS | 영상 오더를 만들면 PACS 워크리스트에 자동 등록 | `구현·미검증` — 따라가기에서 **완료를 확인하지 못했습니다**([S3](../build-guide/S3-clinical-departments.md)) |
| ⑥ | 촬영 | PACS → 촬영 장비 | 모달리티 워크리스트 · 촬영 상태 보고 · 영상 수신(DICOM) | `구현·미검증` |
| ⑥ | 판독 결과 반영 | PACS → HIS | 판독 결과를 HIS 에 반영하고 오더를 완료로 바꿈 | `구현·미검증` |
| ⑦ | 판독 서명 | PACS → sign | 판독의 본인의 판독보고서 전자서명 | `검증됨`(2026-09-15) |
| ⑧ | 동의서 | HIS → sign · sign → HIS | 동의서 서명 요청 제출 · 서명 완료 통지 | 완료 통지 `검증됨`(2026-09-14) · HIS 화면 경로의 직원 서명은 `구현·미검증`(따라가기에서 끝까지 가지 못함) |
| ⑨ | 수납 · 청구 | HIS ⇄ ERP | 진료비 계산서 조회(ERP 산정값) · 청구 라인 · 미청구분 조회 | `검증됨`(2026-09-15) |
...
| HIS ⇄ AI Server | `구현·미검증` 4 · `미구현` 1 | `미구현`은 관리 화면의 모델 목록 대조입니다. 따라가기에서 요약 · 번역 · 초안 경로를 불러 봤지만 연결 전체를 확인하지 못해 상태는 그대로입니다 |
| HIS ⇄ LIS | `검증됨` 5 · `구현·미검증` 4 · `미구현` 4 | 주 경로(FHIR R4 오더 · 결과 · 취소 · 환자 조회 · 카탈로그 반입)는 실제 호출로 확인했고, HL7 v2 로 가는 **대체 경로**(결과 ORU · 처방 OML · Reflex ORM)와 Reflex 승인 웹훅이 `미구현`입니다 |
| LIS ⇄ PACS | `검증됨` 2 | 병리 워크리스트 · 뷰어 링크 모두 확인 |
| HIS ⇄ PACS | `구현·미검증` 7 · `미구현` 3 · `판정 불가` 1 | `미구현`: 판독 결과 HL7 ORU 송신 · 환자 외부 영상 업로드 중계 · 환자 인구정보 HL7 ADT. `판정 불가`: 영상 바이트 중계(배포 구성에 따름) |
| 검사 장비 ⇄ PACS | `구현·미검증` 1 | — |
| PACS ⇄ sign | `검증됨` 2 | 판독 서명 · 조영제 동의서 환자 서명 |
| HIS ⇄ sign | `검증됨` 3 · `구현·미검증` 1 | `구현·미검증` 은 HIS 화면 경로의 서명요청 제출입니다 — 따라가기에서 끝까지 가지 못했습니다 |
| HIS ⇄ ERP | `검증됨` 6 · `구현·미검증` 8 · `미구현` 1 | `미구현`: 재고 입고 · 자산 코드 매핑 · 청구 심사결과 콜백 등 HIS 가 받을 준비만 된 경로 |
