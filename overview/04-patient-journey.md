...
| HIS ⇄ sign | `검증됨` 3 | 이 표에 **싣지 않은 연결 7개** 가운데 하나가 HIS 화면 경로의 서명요청 제출입니다(담당 확인을 기다리는 중) |
| HIS ⇄ ERP | `검증됨` 5 · `구현·미검증` 6 · `미구현` 1 | `미구현`: 재고 입고 · 자산 코드 매핑 · 청구 심사결과 콜백 등 HIS 가 받을 준비만 된 경로 |
