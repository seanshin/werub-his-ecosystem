...
구축 기관이 계획을 잘못 세우지 않도록, 지금 상태를 있는 그대로 적습니다. 모든 내용은 [통합 릴리즈 `2026.09`](../RELEASES/2026.09/manifest.md)의 버전 조합(계측일 2026-09-11)에서의 사실이고, **시스템 담당 확인 전 · 생태계 자료 측 조사 기준**입니다.
...
| **기관명 설정** | HIS 코드에 병원명이 고정 문자열로 남은 파일이 118개 있습니다(2026-09-17 기준 커밋에서 다시 셈에서 다시 셈 · 09-10 값과 같음). 설정값으로 옮기는 작업이 진행 중입니다 | 끝나기 전까지 자기 병원명을 넣으려면 코드를 고칩니다(S1) |
...
- 통합 릴리즈 번호는 **`2026.09`**(날짜형)로 정해졌고, 첫 릴리즈를 잘랐습니다(2026-09-18). 다음 릴리즈를 언제 자를지는 아직 정하지 않았습니다.
