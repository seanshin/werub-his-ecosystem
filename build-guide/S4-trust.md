...
이 가운데 **새 설치본끼리 실제로 호출해 확인한 것**(연결 표 `검증됨` · 2026-09-14~15)은 다음과 같습니다 — HIS → sign 직원 신원 · 서명 · 오더 서명 로그 봉인, sign → HIS 서명 완료 통지, edu ⇄ sign 이수증 봉인 · 완료 통지, ERP ⇄ sign 외주 계약 서명 · 완료 통지, PACS → sign 판독 서명 · 조영제 동의서 환자 서명. 나머지(Clinic 결재 문서 앵커 · ERP 감사 이벤트 · HIS 화면 경로의 동의서 직원 서명 등)는 `구현·미검증` 입니다([연결 상태](../RELEASES/2026.09/compatibility.md)).
