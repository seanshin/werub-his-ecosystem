...
sign 을 부르는 곳: HIS(동의서 · 발급 문서 · 직원 · 환자 인증서 · 오더 서명 감사) · PACS(판독 서명 · 영상 동의서) · edu(이수증) · ERP(계약 · 감사 이벤트) · Clinic(결재 문서 앵커)입니다.

이 가운데 **새 설치본끼리 실제로 호출해 확인한 것**(연결 표 `검증됨` · 2026-09-14~15)은 다음과 같습니다 — HIS → sign 직원 신원 · 서명 · 오더 서명 로그 봉인, sign → HIS 서명 완료 통지, edu ⇄ sign 이수증 봉인 · 완료 통지, ERP ⇄ sign 외주 계약 서명 · 완료 통지, PACS → sign 판독 서명 · 조영제 동의서 환자 서명. 나머지(Clinic 결재 문서 앵커 · ERP 감사 이벤트 · HIS 화면 경로의 동의서 직원 서명 등)는 `구현·미검증` 입니다([연결 상태](../RELEASES/draft/compatibility.md)).
...
| sign → HIS 서명 완료 통지(2026-09-14 · `검증됨`) | 직원 서명이 끝나면 sign 이 HIS 웹훅으로 알리고, HIS 는 **동의서의 서명 상태를 `COMPLETED` 로 바꾸며 참가자 · 인증서 일련번호 · 키 보관 방식 · 감사 증명 주소**를 함께 남깁니다. 서명(HMAC)이 틀리거나 없는 통지는 **반영되지 않습니다.** 🔸 HIS **화면 경로**(`request-sign`)로 만든 동의서의 직원 서명은 따라가기에서 **끝까지 가지 못했습니다** — 직원 토큰으로 서명하는 단계에서 거부됐습니다. 직원 인증서 발급 뒤 sign 에 직접 요청을 만드는 경로(위 칸)는 끝까지 됩니다. 어떤 조합으로 화면 경로를 열 수 있는지는 **두 프로젝트의 협의가 필요**하며 `확인 필요(따라가기)` 로 둡니다 |
...
| 완료 통지(콜백) 주소 | 🔴 **운영 설정에서는 `https` 주소만 받습니다**(따라가기 2026-09-15 · `http` 콜백은 거부) | 원내 시스템(교육 · ERP 등)이 `http` 로만 떠 있으면 앞단에 TLS 를 두고 **그 인증서를 부르는 쪽이 신뢰하게** 해야 통지가 들어갑니다. 따라가기에서는 원내 전용 인증기관을 만들어 중계를 세웠습니다 → [S5](S5-management.md) |
| 본인확인 공급자 | 계약 전에는 **모의 공급자**로 동작합니다 | 이때 서명 포털이 스스로 **"데모"로 표시**합니다(따라가기에서 확인). 실환자 서명 전에 실제 본인확인 연동을 끝냅니다 |
...
- **마스터키**(`CRYPTO_MASTER_KEY` · 32바이트 난수를 base64 로). 연동 설정에 넣은 벤더 자격증명과 서명 이미지를 이 키로 봉인해 저장하고, 관리자 세션 서명키도 이 키에서 파생합니다. **운영 모드에서 이 값이 없으면 sign 은 기동을 거부합니다**(임시 키로 떠서 조용히 도는 일이 없게).
- **마스터키를 바꾸면 봉인된 연동 설정과 관리자 세션이 모두 무효가 됩니다** — 연동 설정 재입력 · 관리자 재로그인이 뒤따릅니다. 저장소의 개통 런북은 팀별 API 키 · 웹훅 비밀은 **신키 유예 → 상대 팀 전환 회신 → 구키 폐기** 순서로 무중단 교체하도록 적어 두었지만, **마스터키는 그 절차의 대상이 아니며 별도 절차로 남겨져 있습니다** — 기관이 교체 시점과 순서를 정합니다(2026-09-16 기준 커밋 확인).
- **백업 암호화 비밀.** 저장소의 백업 스크립트는 DB 덤프를 컨테이너 안에서 만들어 목차까지 검증한 뒤 반출하고, 체크섬을 남긴 다음 **GPG 대칭키(AES-256)로 암호화하고 평문을 즉시 지웁니다.** 암호는 호스트의 파일(권한 600)에서 읽습니다. 보존 세대는 기본값으로 **로컬 14 · 오프사이트 30** 입니다.
  - 🔴 **이 암호를 잃으면 백업 전체를 열 수 없습니다.** 그래서 **서버 밖에도** 보관합니다(저장소 런북은 사본 3곳 — 라이브 · 오프사이트 · 비밀번호 관리자 — 을 필수로 적습니다).
  - 백업이 멎은 것은 **성공 신호의 부재**로 판정합니다. 백업이 끝나면 앱에 성공 하트비트를 보내고, 그 하트비트가 기준 시간(기본 26시간)을 넘겨 오지 않으면 중단으로 봅니다. 오프사이트 전송 실패는 백업 실패로 세지 않습니다(진짜 원인이 가려지지 않게).
  - 인증 기관(CA) · 타임스탬프 키는 볼륨의 키 보관소에 있고 **재기동 뒤에도 남아야 하므로 볼륨 보존이 필수**이며, 그 자체가 정기 백업 대상입니다.
- 복구는 **해 보기 전까지 된다고 말하지 않습니다.** 저장소 런북은 분기 1회 복구 리허설을 두고, **복원본 위에서 감사 체인 검증이 통과하는 것**을 성공 기준으로 삼습니다(따라가기에서는 HIS 쪽 백업 → 다른 DB 복원을 시험했습니다 → [S8](S8-go-real.md)).
