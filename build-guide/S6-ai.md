...
3. **모델 반입** — 격리 구간 밖에서 Ollama 로 모델을 받아 모델 저장소(볼륨 · 디렉터리)째 옮깁니다. 기본 라우팅이 부르는 모델 이름(범용 14B 급)이 없으면 요청이 실패합니다. 🔴 **디스크를 넉넉히** — 따라가기에서 모델 · 엔진 이미지를 받다가 가상 머신 디스크가 가득 차 **같은 호스트의 HIS DB 가 잠시 복구 모드로 들어갔습니다**.
   - 🔴 **임베딩 모델도 함께 받습니다.** 생성 모델만 있으면 문장 생성은 되지만 **자료를 색인하는 경로가 실패**합니다 — 따라가기에서 edu 학습 자료 색인과 ERP 보험 공시 색인이 둘 다 이 이유로 끝나지 못했습니다(호출 · 실패 처리까지는 정상이었습니다).
   - 🔴 **임베딩 모델은 이름이 코드에 고정돼 있습니다**(BGE-M3 · 기준 커밋). 다른 임베딩 모델을 대신 받아 두어도 **그 모델을 부르지 않습니다** — 바꾸려면 코드를 고쳐야 합니다. 받을 모델 이름을 먼저 확인하십시오.
   - **임베딩 요청은 컨텍스트 길이를 따로 작게 줍니다**(기본 2048 · `RAG_EMBED_NUM_CTX`). 전역 컨텍스트 길이(`OLLAMA_CONTEXT_LENGTH`)와 동시 요청 수를 크게 잡아 두면, 임베딩 모델이 VRAM 을 수 GB 로 부풀려 **색인이 실패**합니다. 색인만 실패하고 생성은 되는 증상이면 이 값을 먼저 봅니다.
   - **역할마다 부르는 모델이 다릅니다.** 코드가 요약 · 태그 · 번역 · 질의응답에 한 모델을, 감성 · 분류에 더 작은 모델을 추천값으로 두고 있습니다. **"모델 하나만 받으면 된다"가 아닙니다** — 쓰려는 기능이 부르는 이름을 모두 받아야 합니다.
4. **기동** — `gunicorn -c gunicorn.conf.py app:app`. 🔴 앞에 프록시를 두면 **`X-Forwarded-For` 헤더를 반드시 붙이도록** 설정합니다(따라가기에서 확인 — 설정 방법은 AI Server 저장소 운영 안내를 따릅니다). 상태 점검 `GET /api/health` 가 항목별로 답합니다. 기동 직후 `tunnel: down` 은 `ingress_mode` 기본값(`ssh_tunnel`) 때문입니다(아래 설정). 🔴 **기동하면 쌓여 있던 작업이 바로 시작됩니다** — 따라가기에서는 모델을 넣기 전에 띄운 탓에 대기 작업 수십 건이 재시도 끝에 모두 실패로 끝났습니다. **모델을 먼저 넣고 띄우거나**, 먼저 띄웠다면 실패한 작업을 확인하고 정리합니다.
...
- **의료 데이터 정본** — 의약품 · DUR · 코드 · 수가 정본을 바깥에서 받아 옵니다. 기준 커밋의 반입 경로는 두 갈래이고 **둘 다 바깥을 직접 부릅니다**(2026-09-16 확인): ① 심사평가원 게시판에 월별로 올라오는 목록 파일을 주기적으로 내려받는 경로 ② 공공 데이터 API 를 **서비스 키**로 부르는 경로.
  - 갱신은 받아서 바로 덮지 않습니다 — 내려받기와 파싱이 성공하고 **건수가 최소 기준을 넘을 때만** 정본을 교체하고, 사이트 장애나 파일 이상이면 **기존 정본을 그대로 둡니다.** 같은 판을 다시 적재하지 않도록 판 번호로 거릅니다.
  - 🔴 **파일을 받아 넣는 반입 경로는 기준 커밋에 없습니다.** 그래서 밖으로 나갈 수 없는 망에서는 ① 이 주소들만 예외로 여는 것 ② 기관이 반입 절차를 따로 만드는 것 중 하나를 골라야 합니다. 어느 쪽이든 **바깥 주소가 필요한 기능**이므로 [S0 의 대외 발신 정리](S0-prepare.md)에 함께 적습니다.
...
각 단계를 **어느 화면에서** 하는지는 다음과 같습니다(기준 커밋 · 2026-09-16 확인). **누가** 정하는지는 결정마다 다르므로 [결정 체크리스트](../checklist/decisions.md)의 「결정권자」 칸을 봅니다 — 선행 결정 셋은 개설자 · 원장 · 법무 · 정보보호책임자가, 켤 범위는 위원회가 정하도록 적혀 있습니다.

| 단계 | 화면 | 비고 |
|---|---|---|
| 1 · 선행 결정 기록 | 결정 등록부 `/admin/decisions` | 등록부는 결정을 **기록**합니다. `legal.phiBoundary.gpuTier` 처럼 **기록해도 배포로만 반영되는 것**이 있어, 등록부의 「반영 방식」 칸을 함께 봅니다 |
| 2 · 켤 범위 의결 | 결정 등록부 `/admin/decisions` | 위원회 의결 항목입니다. 등록부에 기록해도 **안건 상신 · 의결은 따로 남습니다** |
| 3 · AI 서버 연결 | 시스템 설정 `/admin/ai-settings` | 🔴 **값을 고치는 화면은 이쪽 하나뿐입니다.** `/admin/config` 는 같은 구성 저장소를 **읽어 보여 주기만** 하고 고치는 경로가 없습니다 |
| 4 · 기능 스위치 | 시스템 설정 `/admin/ai-settings` → 개원 관제 `/admin/go-live` → AI 감독 관제 `/admin/ai-oversight` | 스위치를 켠 뒤 Go-Live 항목 표시를 확인하고, 감독 화면에서 제안 · 승인 기록이 남는지 봅니다 |
| 5 · 순서 기록 | (기관 문서) | 켠 순서와 날짜를 남기는 자리는 제품 안에 없습니다 — 기관 문서에 적습니다 |
...
- **HIS 에서 이 동의를 받고 기록하는 자리는 환자 상세 화면입니다**(기준 커밋 · 2026-09-16 확인). 동의가 없으면 그 자리에 위험 카드 · 트윈 뷰어 대신 **「활용 동의가 필요합니다」 안내와 `동의 기록` 단추**가 나오고, 동의가 있으면 같은 자리에서 **`동의 철회`** 를 누를 수 있습니다. 철회하면 카드와 뷰어가 다시 잠깁니다.
  - 기록에는 **동의 여부 · 동의 시각 · 철회 시각 · 기록한 직원**이 남고, 동의를 받은 방식도 함께 저장됩니다. 다만 **화면에서 누르는 경로는 「구두」로만 기록합니다** — 서면 · 포털로 받은 것을 구분해 남기려면 그 경로를 따로 마련해야 합니다.
  - 🔴 **이 동의는 전자서명(S4) 봉투를 만들지 않습니다.** 환자 서명이 붙은 동의서가 필요하면 [S4](S4-trust.md) 의 동의서 경로를 따로 씁니다.
  - 동의는 **막는 쪽으로 동작합니다** — 동의가 없으면 환자 정보를 외부 앱(트윈 · cerno)으로 넘기는 실행과 트윈의 되쓰기가 거부됩니다.
...
| 아직 안 되는 것 | 대체 수단 |
|---|---|
| HIS 관리 화면의 모델 레지스트리 대조 · PACS 의 AI Server 가용성 감시가 `미구현` 입니다. 따라가기(2026-09-15)에서도 두 경로 모두 **AI Server 에서 쓸 수 있는 답을 받지 못했습니다** — 어느 주소로 무엇을 확인할지 두 프로젝트가 맞춰야 합니다 | AI Server 쪽 지표와 상태 점검(`GET /api/health`)으로 봅니다. 🔴 이 두 표시는 **AI 가 살아 있는지 판단하는 근거로 쓰지 않습니다**(따라가기에서 쓸 수 있는 답을 받지 못했습니다). AI 기능 자체를 한 건 불러 확인합니다 |
...
- **검사 통과 표시를 "임상적으로 맞다"로 읽는 것.** AI 초안에 붙는 검사는 **형식과 표현**을 봅니다. 따라가기에서 작은 대체 모델이 증상과 동떨어진 초안을 냈을 때도 형식 검사는 통과로 표시됐습니다 — **초안은 사람이 읽고 판단한다는 전제**가 그대로 필요합니다.
