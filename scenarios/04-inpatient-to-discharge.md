...
이미지는 아직 넣지 않습니다. 데모 병원 이름을 정하고 가상 데이터로 찍은 뒤, [캡처 대장](../assets/CAPTURE-LEDGER.md)에 적고 사람이 확인합니다.

| 자리 | 담을 화면 | 시스템 · 화면 | 조건 |
|---|---|---|---|
| 04-01 | 병상 · 입원 보드와 입원 수속 | HIS `병상·입원 보드` · `입원` | — |
| 04-02 | 수술 동의서 서명 요청과 서명 완료 상태 | HIS `동의서` · sign 서명 화면 | sign 연결을 모의가 아닌 모드로 |
| 04-03 | 수술 준비 | HIS `수술 준비` | — |
| 04-04 | 병리 슬라이드 현미경 모드 | PACS 웹 뷰어 · LIS 병리 | — |
| 04-05 | 회복실 | HIS `회복실(PACU)` | — |
| 04-06 | 바코드 투약 확인과 투약 기록(MAR) | HIS `간호(입원)` | — |
| 04-07 | 차트의 twin 위험 카드 | HIS 차트 · twin | twin 을 연결한 설치본 |
| 04-08 | twin 의 AI 설명 초안(`machine-generated` 표시)과 "차트 저장" | twin 환자 화면 | AI 기능을 켠 설치본 |
| 04-09 | 퇴원 요약 초안 승인(AI 표기와 면책 문구가 보이게) | HIS `퇴원` | AI 기능을 켠 설치본 |
| 04-10 | 중간정산 · 퇴원 수납 | HIS `수납` · `원무·수납 보드` | — |
| 04-11 | 진단서 발급과 전자서명 | HIS `진단서 발급` | 발급 문서에 데모 병원 이름만 보이는지 확인 |
