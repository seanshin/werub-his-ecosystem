...
- 연결 상자의 테두리: 실선 = `구현·미검증` · 주황 파선 = `미구현` · 보라 점선 = `판정 불가`. `검증됨`(실제 호출 확인 · 굵은 실선)은 14개입니다 — HIS ⇄ sign 직원 서명 · 완료 통지 · 오더 로그 봉인 · HIS ⇄ LIS 검사 오더 · 환자 조회 · 검사 결과 · 오더 취소 · HIS → ERP 직원 SSO · HIS ⇄ edu 직원 SSO · 공개키 조회 · 직원 명부 · 이수 기록 · edu ⇄ sign 이수증 서명 · 완료 통지(2026-09-14~15).
