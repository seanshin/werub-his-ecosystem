...
| 연결 | 113개 — 검증됨 3 · 구현·미검증 85 · 설계만 1 · 미구현 15 · 중단 7 · 판정 불가 2 |
...
flowchart LR
  ...
  APP["환자 앱"]
  HIS ---|"구현·미검증 14 · 미구현 1"| ERP
  HIS ---|"검증됨 2 · 구현·미검증 7 · 미구현 4"| LIS
  HIS ---|"구현·미검증 10 · 미구현 1 · 판정 불가 1"| CLINIC
  HIS ---|"구현·미검증 7 · 미구현 3 · 판정 불가 1"| PACS
  HIS ---|"구현·미검증 6 · 미구현 1"| EDU
  HIS ---|"구현·미검증 6"| TWIN
  HIS ---|"구현·미검증 4 · 미구현 1"| AIS
  ERP ---|"구현·미검증 3 · 미구현 1"| SIGN
  HIS ---|"검증됨 1 · 구현·미검증 3"| SIGN
  HIS ---|"중단 3"| JITSI
  LIS ---|"구현·미검증 2"| PACS
  PACS ---|"구현·미검증 2"| SIGN
  EDU ---|"구현·미검증 2"| SIGN
  ...
  linkStyle 5,8,10,11,12,15,17,18,19,20,21,22,24,25,26,27,28,32 stroke:#4a6fa5,stroke-width:2px
...
|---:|---|---:|---|---|---|
| 1 | HIS ⇄ ERP | 15 | HIS → ERP 6 · ERP → HIS 9 | 구현·미검증 14 · 미구현 1 | `미구현` |
| 2 | HIS ⇄ LIS | 13 | HIS → LIS 4 · LIS → HIS 9 | 검증됨 2 · 구현·미검증 7 · 미구현 4 | `미구현` |
...
| 5 | HIS ⇄ edu | 7 | HIS → edu 2 · edu → HIS 5 | 구현·미검증 6 · 미구현 1 | `미구현` |
| 6 | HIS ⇄ twin | 6 | HIS → twin 3 · twin → HIS 3 | 구현·미검증 6 | `구현·미검증` |
| 7 | HIS ⇄ AI Server | 5 | HIS → AI Server 5 | 구현·미검증 4 · 미구현 1 | `미구현` |
| 8 | ERP ⇄ sign | 4 | ERP → sign 3 · sign → ERP 1 | 구현·미검증 3 · 미구현 1 | `미구현` |
| 9 | HIS ⇄ sign | 4 | HIS → sign 3 · sign → HIS 1 | 검증됨 1 · 구현·미검증 3 | `구현·미검증` |
| 10 | HIS ⇄ Jitsi | 3 | HIS → Jitsi 3 | 중단 3 | `중단` |
| 11 | LIS ⇄ PACS | 2 | LIS → PACS 2 | 구현·미검증 2 | `구현·미검증` |
| 12 | PACS ⇄ sign | 2 | PACS → sign 2 | 구현·미검증 2 | `구현·미검증` |
| 13 | edu ⇄ sign | 2 | edu → sign 1 · sign → edu 1 | 구현·미검증 2 | `구현·미검증` |
