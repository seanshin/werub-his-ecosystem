...
**처음이라면 이 셋만 보세요** — [한 문장](overview/01-one-sentence.md) · [지금 상태](overview/08-status-and-preparation.md) · [화면 293장](screens/)
...
- **HIS 설치 경로** — 🔴 저장소의 운영용 컨테이너 설치 파일(compose · Dockerfile)이 **그대로는 동작하지 않습니다**(새 설치본 따라가기 2026-09-13 · 빌드 메모리 · 누락 의존성 · 스키마 반영 명령 · 내부 바인딩). 첫 관리자 계정도 **데모 시드로만** 만들어집니다. 따라가기에서 한 우회와 순서는 [S1](build-guide/S1-core-his.md#설치-순서)에 있습니다. sign 은 이미지가 그대로 빌드되고 기동합니다([S4](build-guide/S4-trust.md#②-설치)).
