...
|---|---|---|
| 병원장·CIO | 왜 이 구조인가? 무엇을 얻고 무엇을 감수하나? 구축 규모와 비용은? | [발표 덱](deck/) → [개요서 2 · 8장](overview/) |
| 전산·인프라팀 | 무엇을 어디에 설치하나? 서버·GPU·DB 요구사항은? | [구축 가이드 S0](build-guide/S0-prepare.md) → [시스템 구성서](systems/) → [배포 구성](diagrams/deployment.md) |
...
| 연동 개발자·파트너 | 시스템끼리 무슨 프로토콜·인증으로 붙나? 무엇이 검증됐나? | [연동 계약 지도](integration/) → [연결 상태 표](RELEASES/draft/compatibility.md) |
...
|---|---|---|
| [S1](build-guide/S1-core-his.md) 코어 HIS | HIS API · 웹 · PostgreSQL · Redis | 🔴 운영 compose · Dockerfile 이 **그대로는 동작하지 않음** — 빌드 메모리 부족 · 선언 안 된 의존성 · 스키마 반영 명령(`migrate deploy` 가 아니라 `db push`) · API 가 컨테이너 안에서 `127.0.0.1` 에만 붙음. **첫 관리자는 기본 시드로만** 생김. 완료 조건 화면으로 판정: 개원 완료 0 · Go-Live 19%(6/31) |
...
| [S3](build-guide/S3-clinical-departments.md) PACS | Orthanc · 백엔드 · 작업자 등 12 컨테이너 | 이미지 태그 · TLS 파일 · 헬스체크 · 기본 테이블 생성 단계를 우회해 설치. LIS 와의 병리 워크리스트 · 뷰어 링크, PACS → sign 판독 서명 · 동의서 서명은 확인. 🔴 **HIS 와의 영상 경로**(워크리스트 등록 · 영상 조회 · 판독 작성/서명 · 영상 바이트 중계 · 뷰어 진입)는 **완료를 확인하지 못했습니다** — 코드 수정이나 프로젝트 사이 협의가 필요합니다 |
...
| [S2](build-guide/S2-patient-access.md) 환자 접점 | 설치하지 않음 | 환자 앱 · 공개 홈페이지는 이번 따라가기 범위 밖입니다(HIS 쪽 수신부만 코드로 대조) |
...
전체 표는 [연결 상태](RELEASES/draft/compatibility.md)(확인일 칸)에 있습니다. 나머지 `구현·미검증` 61개는 **양쪽 코드가 맞물려 있다는 뜻이지 동작한다는 뜻이 아닙니다** — 기관이 [S7 리허설](build-guide/S7-rehearsal.md)에서 쓰는 연결마다 같은 방식으로 확인합니다.
...
- **HIS 설치 경로** — 🔴 저장소의 운영용 컨테이너 설치 파일(compose · Dockerfile)이 **그대로는 동작하지 않습니다**(새 설치본 따라가기 2026-09-13 · 빌드 메모리 · 누락 의존성 · 스키마 반영 명령 · 내부 바인딩). 첫 관리자 계정도 **데모 시드로만** 만들어집니다. 따라가기에서 한 우회와 순서는 [S1](build-guide/S1-core-his.md#설치-순서)에 있습니다. sign 은 이미지가 그대로 빌드되고 기동합니다([S4](build-guide/S4-trust.md#②-설치)). 다른 시스템에서 나온 것은 [따라가 본 결과](#새-설치본으로-따라가-본-결과)에 모았습니다.
...
| `tools/` | 공개 검사기 · 규모 계측기 · 체크리스트 추출기 · 매니페스트 생성기 · 연결 지도 · 연동 매트릭스 생성기 · HIS 메뉴 추출기 · 캡처 목록 생성기 · 릴리즈 기록 계수기 · 바꿔야 할 코드 기본값 목록 생성기 · 문서 간 일치 검사기 — 발행 전 한 번에 `node tools/verify-all.mjs`(검사 14개) | ✅ |
...
| **구축을 앞두고** | [따라가 본 결과](#새-설치본으로-따라가-본-결과) → [구축 가이드](build-guide/) S0~S8 + [체크리스트](checklist/) 세 벌(개원 60 · 개시 점검 60 · 사람 결정 56) |
