...
이 문서에 **적지 않은 것**: 각 시스템이 링크하는 라이브러리 의존성(npm·pip 패키지)과 런타임 기반 이미지(Node.js·Python·Alpine 등). 시스템별 의존성 목록은 이 릴리즈에 담지 않았습니다(만드는 시점은 정해져 있지 않습니다).
...
| Orthanc 코어 | PACS | 이미지 `orthancteam/orthanc:24.12.2` · 코어 판본 미확정 — 코드 선언은 `1.12.11`, 태그 계열이 담는 것은 1.12.5(주 1) | `GPL-3.0-or-later` | [COPYING 1.12.5](https://orthanc.uclouvain.be/hg/orthanc/file/Orthanc-1.12.5/COPYING) · [라이선스 안내](https://orthanc.uclouvain.be/book/faq/licensing.html) | 2026-09-11 | 원문 확인 · 판본 확인 필요 |
...
생태계는 공공기관이 배포하는 코드와 기준 데이터를 반입해서 씁니다. **이 저장소는 그 데이터를 담지 않습니다. 구축 기관이 배포 기관에서 직접 받아 반입합니다.** 반입 절차는 [구축 가이드](build-guide/) S1(코어 HIS)과 S6(AI 계층)에서 안내합니다 — 🔴 기준 커밋에는 **파일을 올려 반입하는 자리가 없고**, 국가 축도 없습니다(두 문서에 무엇이 없는지 적어 두었습니다).
