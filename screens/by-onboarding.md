...
> 회색으로 덮인 자리는 **개발 전용 테스트 계정 목록**입니다. 리허설 설치본에만 있는 것이라 공개 자료에서는 가렸습니다.
...
- **퇴직** — 직원 상태를 바꾸고 역할 · 접근 권한을 회수하고 인증서를 폐기하고 등록 단말을 해제합니다. 🔴 **형제 시스템의 계정 · 세션 회수 방식은 시스템마다 다릅니다** — 각 시스템에서 따로 확인합니다(`확인 필요(따라가기)`).
...
- 로그인의 2단계 인증 · 패스키 여부, 초대 기한, 형제 시스템 계정 회수 절차는 `확인 필요(따라가기)` 입니다.
