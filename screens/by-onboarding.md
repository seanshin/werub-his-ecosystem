...
직원마다 **역할 · 진료과 · 세부전문과 · 상태**가 붙습니다. 탭은 `직원 목록` · `조직도` · `직원 등록` · `Clinic 연동` 이고, 오른쪽 위의 **`Clinic 동기화`** 가 그룹웨어로 직원을 내보내는 자리입니다(HIS → Clinic 직원 일괄 등록 · [연결 상태](../RELEASES/2026.09/compatibility.md) `구현·미검증`).
...
- **2단계 인증 · 패스키는 있습니다**(기준 커밋 확인 · 2026-09-17). HIS 는 **패스키(WebAuthn)** 를 쓰고, 위험한 동작 앞에서는 **단계 상승 재인증**을 요구합니다 — 그 재인증 토큰은 **동작 · 대상 · 요청 본문에 묶이고 한 번만 쓰이며 5분 뒤 만료**됩니다(같은 토큰으로 다른 것을 승인할 수 없게). PC 에서 요청하고 **직원 휴대전화의 생체 승인**으로 통과하는 경로도 있습니다. 문자 · 이메일 일회용 번호 방식은 이 확인에서 찾지 못했습니다.
- **초대 기한은 14일입니다**(기준 커밋 확인 · 2026-09-22). 초대를 만든 시점부터 14일 뒤가 만료 시각으로 박히고, 지나면 주기 작업이 상태를 **`만료` 로 바꿉니다 — 지우지는 않습니다.** 재발급하면 **이전 링크가 그 자리에서 무효**가 되고 기한이 다시 14일로 늘어납니다. 🔴 이 14일은 **코드에 상수로 박혀 있어 설정 화면에서 바꿀 수 없습니다** — 기관이 다른 기한을 쓰려면 코드를 고쳐야 합니다. 🔴 초대 **메일**은 이메일 발송 제공자가 등록돼 있어야 실제로 나갑니다 — 없으면 **「미발송」으로 적히고 발송으로 세지 않습니다**(QR · 링크는 애초에 관리자가 손으로 전달한 기록으로 따로 구분됩니다).
- 형제 시스템의 계정 회수는 **경로가 있는지까지만** 위에 적었고, **실제로 회수해 본 기록은 없습니다.**
