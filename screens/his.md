...
> **EN** — The HIS ships **266 screens** in its default menu, grouped into 8 domains and 26 menu clusters. This chapter walks the domains and shows what each screen is for. Menu names and default roles are **code defaults** — an administrator can rename, reorder, hide or add menus. A screen existing is not the same as a connection being verified: see [connection status](../RELEASES/2026.09/compatibility.md) (**27 verified** with a date — 23 of them also re-tested with a deliberately injected fault). Captured so far: **264 of 266** menu screens (2 withheld — infrastructure addresses / partner details), on a rehearsal install running v4.19.0 (base commit: v4.18.0).
...
> 📌 이 자료의 [연결 상태](../RELEASES/2026.09/compatibility.md)는 대부분 **코드 대조**로 판정한 것입니다(`검증됨` 은 새 설치본 따라가기에서 확인한 27개). 이 콘솔은 그 판정을 **실제 호출로 바꿀 수 있는 자리**입니다 — 구축 기관이 리허설(S7)에서 쓰는 도구이기도 합니다.
...
> 둘 다 제목이 「시스템 설정」입니다. 가이드를 따라 "시스템 설정 화면에서 AI 스위치를 끄세요"를 하려는 사람은 **메뉴를 눌러서는 다른 화면에 도착합니다.** (2026-09-12 두 화면 모두 확인)
>
> **어느 쪽이 정본인가 — 코드에서 확인했습니다**(기준 커밋 · 2026-09-16). **정본은 화면이 아니라 설정 키 자체(`ai.*`)입니다.** 두 화면은 같은 구성 저장소를 보고 있고, **값을 고치는 쪽은 `/admin/ai-settings` 하나뿐**입니다 — 이 화면이 스위치를 켜고 끌 때 구성 키 쓰기 API 를 그대로 부릅니다. `/admin/config` 는 **목록을 읽어 보여 줄 뿐 고치는 경로가 없습니다**(조회만 합니다).
> 그래서 실무에서는 이렇게 씁니다 — **값을 보려면 `/admin/config`, 바꾸려면 `/admin/ai-settings`.** 두 화면의 값이 달라 보일 일은 없습니다(같은 키를 봅니다).
...
- **연결은 따로 봅니다.** 이 화면들이 다른 시스템과 실제로 맞물리는지는 [연결 상태](../RELEASES/2026.09/compatibility.md)에 있고, 지금 **`검증됨` 은 27** — 목록은 [따라가 본 결과](../build-guide/follow-along-2026-09.md#실제로-호출해-검증됨-을-붙인-연결) 한 곳에 있습니다.
