...
- **따라가기에서 확인한 것**(2026-09-13~16 — [결과](../build-guide/follow-along-2026-09.md)) — ✅ 저장소 파일 그대로 빌드 · 기동됐고 마이그레이션 247개가 적용됐습니다. 다만 **ERP 안에서 첫 관리자를 만드는 경로가 없어** HIS SSO 로 처음 들어올 때 계정이 생깁니다. 순서는 [S5](../build-guide/S5-management.md).
- 🔴 **운영 표시값은 정확히 한 가지 문자열이어야** 합니다. 비슷한 값을 넣으면 안전장치가 조용히 건너뛰어집니다.
- 계약 서명에 쓰는 **첨부 저장소 설정이 운영 compose · 환경 예시에 없습니다.** 넣지 않으면 서명 요청이 거절됩니다.
