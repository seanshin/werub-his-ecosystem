...
- **따라가기에서 확인한 것**(2026-09-13~16 — [결과](../build-guide/follow-along-2026-09.md)) — ✅ 저장소의 **운영용 이미지가 그대로 빌드되고 떴습니다**(생태계에서 드문 경우입니다). 다만 운영 compose 가 **바깥 네트워크 하나를 미리 만들어 두기를 요구**해서, 없으면 기동이 멈춥니다. 자세한 순서는 [S4](../build-guide/S4-trust.md).
- 부르는 쪽에서 **주소 끝의 판본 경로까지** 넣어야 붙습니다. DB 비밀번호를 비워 두면 약한 고정 기본값으로 뜨고, 몇몇 주소 기본값에는 다른 설치본 주소가 들어 있습니다 — 설치할 때 모두 채웁니다.
- **완료 통지(콜백)는 운영 설정에서 `https` 만 받습니다.** 원내 시스템이 `http` 로만 떠 있으면 앞단에 TLS 를 두고 그 인증서를 신뢰시켜야 통지가 들어갑니다.
- 본인확인 사업자와 계약하기 전에는 **모의 공급자**로 동작하고, 서명 포털이 스스로 "데모"라고 표시합니다. 실환자 서명 전에 실제 연동을 끝냅니다.
