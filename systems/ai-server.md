...
> 사실 확인: 미확인 — 생태계 자료 측 조사 기준(시스템 담당 확인 전) · 새 설치본으로 한 번 따라가 봄(2026-09-15 · 개발 PC · GPU 없음 · 격리 네트워크 — [결과](../build-guide/follow-along-2026-09.md))
...
| 런타임 | Python 3 · Flask · Gunicorn. 의존성은 `requirements.txt`(Apple Silicon 은 `requirements-mac.txt`). 권장 Python 판본은 새 설치본으로 확인해 싣습니다 |
| 저장소 | 별도 DB 서버가 필요 없습니다. 벡터 저장소(ChromaDB)는 프로세스 안에 내장되고, 데이터셋 · 작업 큐는 SQLite 파일입니다. 모델 · 색인 · 음성 모델이 차지하는 디스크 용량은 계측해 싣습니다 |
...
연결 상태는 [연결 상태](../RELEASES/draft/compatibility.md)에서 가져왔습니다(코드 대조 · 2026-09-11 · 실제 호출 확인 전). AI Server 는 **불리는 쪽**이며, 나가는 연결로 따로 판정된 것은 없습니다.
