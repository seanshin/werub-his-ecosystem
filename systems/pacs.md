...
- **따라가기에서 확인한 것**(2026-09-13~16 — [결과](../build-guide/follow-along-2026-09.md)) — 설치 스크립트를 네 번 돌려 끝냈습니다. **영상 서버 이미지 태그를 받을 수 없었고**, TLS 파일 · 헬스체크 · 기본 테이블 생성 단계에서 차례로 멈췄습니다. 우회한 순서는 [S3](../build-guide/S3-clinical-departments.md)에 적었습니다.
- 서비스 계정을 두 개 이상 만들려면 **계정마다 메일 주소를 따로 줘야** 합니다(기본값이 겹칩니다).
- LIS 와의 병리 워크리스트 · 뷰어 링크, sign 으로 가는 판독 서명 · 조영제 동의서 서명은 확인했습니다. 🔴 **HIS 화면에서 PACS 를 부르는 경로**(영상 조회 · 판독 작성 · 서명 · 뷰어 열기)는 **완료를 확인하지 못했습니다** — 코드 수정이나 프로젝트 사이 협의가 필요합니다.
...
| 항목 | 값 |
|---|---|
| 소스 링크 | [seanshin/openpacs](https://github.com/seanshin/openpacs) — 공개 예정(정리가 끝나는 대로 열립니다 · 주소는 바뀌지 않습니다) |
